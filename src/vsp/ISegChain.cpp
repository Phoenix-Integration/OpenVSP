...
double IPntGroup::GroupDistFract( IPntGroup* g )
{
	...
	GridDensity* gdp = cfdMeshMgrPtr->GetGridDensityPtr();
...
ISeg* ISeg::Split( Surf* sPtr, vec2d & uw )
{
	...
	{
		Puw* pwa = new Puw( m_SurfA, uwa );			cfdMeshMgrPtr->AddDelPuw( pwa );	
		Puw* pwb = new Puw( m_SurfB, uwb );			cfdMeshMgrPtr->AddDelPuw( pwb );
		IPnt* ip = new IPnt( pwa, pwb );			cfdMeshMgrPtr->AddDelIPnt( ip );
		ip->CompPnt();
...
ISegBox::~ISegBox()
{
	if ( m_SubBox[0] ) delete m_SubBox[0];
	if ( m_SubBox[1] ) delete m_SubBox[1];
}
...
//jrg Why 3D ????
	...
	if ( once )
	{
		fprintf( cfdMeshMgrPtr->m_DebugFile, "ISegChain::AddBorderSplit \n");
		once = false;
	}
	if ( closest_dist < 0.1 )
	{
		fprintf( cfdMeshMgrPtr->m_DebugFile, "  Closest_Dist = %12.10f UW =  %12.10f\n", 
