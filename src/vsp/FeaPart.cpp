...
void FeaNode::WriteNASTRAN( FILE* fp )
{
	...
	if ( fabs(x) < 10.0 )			fprintf(fp, "%8.5f,", x );
	else if ( fabs(x) < 100.0 )		fprintf(fp, "%8.4f,", x );
	else							fprintf(fp, "%8.3f,", x );

	if ( fabs(y) < 10.0 )			fprintf(fp, "%8.5f,", y );
	else if ( fabs(y) < 100.0 )		fprintf(fp, "%8.4f,", y );
	else							fprintf(fp, "%8.3f,", y );
		
	if ( fabs(z) < 10.0 )			fprintf(fp, "%8.5f\n", z );
	else if ( fabs(z) < 100.0 )		fprintf(fp, "%8.4f\n", z );
...
//============================================================================//
//============================================================================//
bool SpliceCompare(const FeaSplice* a, const FeaSplice* b) 
{
...
int FeaSpliceLine::processDragEvent()
{
	...
	if ( m_Mode == NORMAL_MODE &&  m_EditSplice )
	{
		...
		feaMeshMgrPtr->UpdateGUI();
		feaMeshMgrPtr->AircraftDraw();
	}
...
double FeaSpliceLine::MapMouseToChord( double x )
{
	...
	if ( chord < 0.0001 )		chord = 0.0001;
	else if ( chord > 0.9999 )	chord = 0.9999;
...
void FeaSlice::SetEndPoints( vec2d & uwA, vec2d uwB )
{
	...
	{
		uwA[0] += 0.00001;		uwB[0] += 0.00001;
	}
	if ( fabs( uwA[1] - uwB[1] ) < 0.00001 )
	{
		uwA[1] += 0.00001;		uwB[1] += 0.00001;
	}

	//==== Find Starting Section ====//
	int sectID = (int)uwA[0];
	vec3d norm = feaMeshMgrPtr->GetNormal( sectID );
	m_Normal = norm;

	//==== Find Bounding Box ====//
	bbox box;
	vec3d lpnt1 = feaMeshMgrPtr->ComputePoint( uwA, true );
	vec3d lpnt2 = lpnt1 + norm;
	for ( int i = 0 ; i < 11 ; i++ )
	{
		vec2d uw = uwA + (uwB-uwA)*(double)i*0.1;
		vec3d pnt = feaMeshMgrPtr->ComputePoint( uw, true );
		box.update( proj_pnt_on_line( lpnt1, lpnt2, pnt ) );
		pnt = feaMeshMgrPtr->ComputePoint( uw, false );
		box.update( proj_pnt_on_line( lpnt1, lpnt2, pnt ) );
	}

	double scale = 1.1*box.diag_dist();

	m_UpperEndPnts[0] = lpnt1;
	m_UpperEndPnts[1] = feaMeshMgrPtr->ComputePoint( uwB, true );

	//==== Find Corner Points & Slightly Expand====//
	vec3d pA = feaMeshMgrPtr->ComputePoint( uwA, true );
	vec3d pB = feaMeshMgrPtr->ComputePoint( uwB, true );
	vec3d off = pA - pB;
//	off.normalize();
	...
	double maxu = (double)feaMeshMgrPtr->GetNumSections() - 0.0001;
	...
	m_UpperStartChainPnt = feaMeshMgrPtr->ComputePoint( uwB, true );
	m_LowerStartChainPnt = feaMeshMgrPtr->ComputePoint( uwB, false );
	m_UpperEndChainPnt = feaMeshMgrPtr->ComputePoint( uwA, true );
	m_LowerEndChainPnt = feaMeshMgrPtr->ComputePoint( uwA, false );
...
{
	int num_sections = feaMeshMgrPtr->GetNumSections();
	...
	if ( IsCap() )
	{
		...
		{
			feaMeshMgrPtr->LoadCapChains( m_CapUpperSurf[0], m_CapUpperSurf[1], upper_chain_list );
			feaMeshMgrPtr->LoadCapChains( m_CapLowerSurf[0], m_CapLowerSurf[1], lower_chain_list );
		}
		else if ( m_CapInFlag )		// Inside Edge
		{
			feaMeshMgrPtr->LoadCapChains( m_CapUpperSurf[0], 0.0, upper_chain_list );
			feaMeshMgrPtr->LoadCapChains( m_CapLowerSurf[0], 0.0, lower_chain_list );
		}
		else						// Outside Edge
		{
			feaMeshMgrPtr->LoadCapChains( m_CapUpperSurf[0], 1.0, upper_chain_list );
			feaMeshMgrPtr->LoadCapChains( m_CapLowerSurf[0], 1.0, lower_chain_list );
		}
	}
	else
	{
		...
		{
			feaMeshMgrPtr->LoadChains( m_Surf, true, i, upper_chain_list );
			feaMeshMgrPtr->LoadChains( m_Surf, false, i, lower_chain_list );
		}
	}
...
int FeaSlice::ComputeNumDivisions()
{
	...
	GridDensity* grid_density = feaMeshMgrPtr->GetGridDensityPtr();
	int max_num = 0;
...
{
	WingSection* sectPtr = feaMeshMgrPtr->GetWingSection( m_SectID );
	...
	vec3d pntIn = feaMeshMgrPtr->ComputePoint( uwIn, true );
	...
	if ( !m_AbsSweepFlag )
	{
		...
		vec3d pntOut = feaMeshMgrPtr->ComputePoint( uwOut, true );

		vec3d vio = pntOut - pntIn;
		vec3d norm = feaMeshMgrPtr->GetNormal( m_SectID );
	...
	vec2d uw;
	vector< vec2d > uwVec;
	for ( int s = 0 ; s < feaMeshMgrPtr->GetNumSections() ; s++ )
	{
		if ( !m_TrimFlag || s == m_SectID )
		{
			sectPtr = feaMeshMgrPtr->GetWingSection( s );
	...
	{
		vec3d pnt = feaMeshMgrPtr->ComputePoint( uwVec[i], true );
...
void FeaRib::ComputeEndPoints()
{
	...
	vec3d pntLE = feaMeshMgrPtr->ComputePoint( uwLE, true );
...
void FeaSkin::WriteData( xmlNodePtr root )
{
	...
	xmlAddIntNode( root, "ExportFlag", m_ExportFlag );
	xmlAddDoubleNode( root, "DefaultThick", m_DefaultThick );
	xmlAddDoubleNode( root, "Density", m_Density );
	...
	for ( i = 0 ; i < (int)m_SpliceLineVec.size() ; i++ )
	{
		...
		xmlAddDoubleNode( sl_node, "PerSpan",		m_SpliceLineVec[i]->m_PerSpan );
		...
		for ( j = 0 ; j < (int)m_SpliceLineVec[i]->m_SpliceVec.size() ; j++ )
		{
			...
			xmlAddIntNode( slice_node, "FixedFlag", splice->m_FixedFlag );
			xmlAddDoubleNode( slice_node, "Thick",  splice->m_Thick );
			xmlAddDoubleNode( slice_node, "Pos",  splice->m_Pos );
		}
	}
}
...
{
	int i, j;
	m_ExportFlag = (xmlFindInt( root, "ExportFlag", 1 ) != 0);	// Bool
	m_DefaultThick = xmlFindDouble( root, "DefaultThick", m_DefaultThick );
	m_Density = xmlFindDouble( root, "Density", m_Density );
	...
	xmlNodePtr splice_line_list_node = xmlGetNode( root, "Splice_Line_List", 0 );
	if ( splice_line_list_node )
	{
		int num_splice_lines =  xmlGetNumNames( splice_line_list_node, "Splice_Line" );
		for ( i = 0 ; i < num_splice_lines ; i++ )
		{
			xmlNodePtr splice_line_node = xmlGetNode( splice_line_list_node, "Splice_Line", i );
			if ( splice_line_node )
			{
				FeaSpliceLine* sl = new FeaSpliceLine(m_DefaultThick);
				sl->m_PerSpan = xmlFindDouble( splice_line_node, "PerSpan", sl->m_PerSpan );
				m_SpliceLineVec.push_back( sl );

				xmlNodePtr splice_list_node = xmlGetNode( splice_line_node, "Splice_List", 0 );
				if ( splice_list_node )
				{
					int num_splices = xmlGetNumNames( splice_list_node, "Splice" );
					if ( num_splices )
					{
						sl->ClearSpliceVec();
						for ( j = 0 ; j < num_splices ; j++ )
						{
							xmlNodePtr splice_node = xmlGetNode( splice_list_node, "Splice", j );
							if ( splice_node )
							{
								FeaSplice* s = new FeaSplice();
								s->m_FixedFlag = (xmlFindInt( splice_node, "FixedFlag", 0 ) != 0);	// Bool
								s->m_Pos = xmlFindDouble( splice_node, "Pos", 0.0 );
								s->m_Thick = xmlFindDouble( splice_node, "Thick", m_DefaultThick );
...
bool SpliceLinePerSpanCompare(const FeaSpliceLine* a, const FeaSpliceLine* b) 
{
...
void FeaSkin::SetNodeThick()
{
	...
	if ( !m_WingSection )
		return;
	//WingSection* sect = feaMeshMgrPtr->GetWingSection( m_SectID );
	//if ( !sect )
	//	return;
