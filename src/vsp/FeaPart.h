...
{
public:
	FeaNode()				{ m_Index = -1; m_Thick = 0.1; };
	FeaNode( vec3d & p )	{ m_Pnt = p; m_Index = -1; m_Thick = 0.1; }
	virtual ~FeaNode()				{ }
...
public:
	...
	virtual void SetDefaultThick( double t )			{ m_DefaultThick = t; }
...
public:
	...
	void SetSurf( Surf* s )					{ m_Surf = s; }			// Upper or Lower Wing Surface
	void BuildMesh();
	void SetNodeThick();
	void SetExportFlag( bool f )			{ m_ExportFlag = f; }
	bool GetExportFlag()					{ return m_ExportFlag; }
	void SetWingSection( WingSection* wp )	{ m_WingSection = wp; }

	void SetDefaultThick( double t );
	double GetDefaultThick()				{ return m_DefaultThick; }
	void SetDensity( double d )				{ m_Density = d; }
	double GetDensity()						{ return m_Density; }
	...
	int  GetCurrSpliceLineID()				{ return m_CurrSpliceLineID; }
...
public:
	...
	virtual void SetNormal( vec3d & norm )		{ m_Normal = norm; }

	virtual void FindUpperLowerPoints();
	virtual int  ComputeNumDivisions();
	virtual void SetNumDivisions( int n )		{ m_NumDivisions = n; }
	...
	virtual bool IsCap()				{ return false; }
