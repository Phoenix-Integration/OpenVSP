...
void Mesh::ColorTris()
{
	...
	for ( t = triList.begin() ; t != triList.end(); t++ )	
	{
		...
		if ( q > M_PI/6.0 )												// > 30 Deg
			...
			{(*t)->rgb[2] = 255; (*t)->rgb[0] = (*t)->rgb[1] = 100; }	// 25 deg
		else															
			{ (*t)->rgb[0] = 255; (*t)->rgb[1] = (*t)->rgb[2] = 100; }	
	}
...
void Mesh::SplitEdge( Edge* edge, Surf* surfPtr )
{
	...
	if ( ta )
	{
		...
		if ( ea0->t0 == ta )		ea0->t0 = ta0;
		else if ( ea0->t1 == ta )	ea0->t1 = ta0;
		else assert(0);

		if ( ea1->t0 == ta )		ea1->t0 = ta1;
		else if ( ea1->t1 == ta )	ea1->t1 = ta1;
		else assert(0);
	...
	if ( tb )
	{
		...
		if ( eb0->t0 == tb )		eb0->t0 = tb0;
		else if ( eb0->t1 == tb )	eb0->t1 = tb0;
		else assert(0);

		if ( eb1->t0 == tb )		eb1->t0 = tb1;
		else if ( eb1->t1 == tb	)	eb1->t1 = tb1;
		else assert(0);
...
void Mesh::SwapEdge( Edge* edge )
{
	...
	if ( ea1->t0 == ta )		ea1->t0 = tb;
	else if ( ea1->t1 == ta )	ea1->t1 = tb;
	else assert(0);
...
//		pc  = (n0->pnt + n1->pnt)*0.5;
	...
	if ( ea0->border || ea1->border )	eca->border = true;
	if ( eb0->border || eb1->border )	ecb->border = true;
	if ( ea0->ridge  || ea1->ridge )	eca->ridge = true;
	if ( eb0->ridge  || eb1->ridge )	ecb->ridge = true;
...
//jrg Check for invalid tris and improved qual
	...
	if ( ta0 )	ta0->ReplaceEdge( ea0, eca );
	if ( ta1 )	ta1->ReplaceEdge( ea1, eca );
	if ( tb0 )  tb0->ReplaceEdge( eb0, ecb );
	if ( tb1 )  tb1->ReplaceEdge( eb1, ecb );
...
//CheckValidTriInput( uw_points, segs_indexes );
	...
	{
		in.pointlist[cnt] = uw_points[j].x();	cnt++;
		in.pointlist[cnt] = uw_points[j].y();	cnt++;
	}
...
assert ( in.segmentlist[cnt] < num_pnts );
		in.segmentlist[cnt] = segs_indexes[j].m_Index[0];	cnt++;
assert ( in.segmentlist[cnt] < num_pnts );
		in.segmentlist[cnt] = segs_indexes[j].m_Index[1];	cnt++;
	}
	...
	double uw_area = (box.get_max(0)-box.get_min(0))*(box.get_max(1)-box.get_min(1));
...
//	sprintf(str, "zpYYQ" );
	...
	for ( i = 0 ; i < out.numberoftriangles ; i++ )
	{
		...
		if ( !e0 )	e0 = AddEdge( n0, n1 );

		Edge* e1 = FindEdge( n1, n2 );
		if ( !e1 )	e1 = AddEdge( n1, n2 );

		Edge* e2 = FindEdge( n2, n0 );
		if ( !e2 )	e2 = AddEdge( n2, n0 );

		Tri* tri = AddTri( n0, n1, n2, e0, e1, e2 );

		if      ( e0->t0 == NULL ) e0->t0 = tri;
		else if ( e0->t1 == NULL ) e0->t1 = tri;
		else assert(0);

		if      ( e1->t0 == NULL ) e1->t0 = tri;
		else if ( e1->t1 == NULL ) e1->t1 = tri;
		else assert(0);

		if      ( e2->t0 == NULL ) e2->t0 = tri;
		else if ( e2->t1 == NULL ) e2->t1 = tri;
		else assert(0);
...
void Mesh::ReadSTL( const char* file_name )
{
	...
	if ( file_id )
	{
		...
		while ( !stopFlag )
		{
			...
			if ( !n0 )	n0 = AddNode( vec3d(v0[0], v0[1], v0[2]), vec2d(0,0) );

			Node* n1 = FindNode( vec3d(v1[0], v1[1], v1[2]) );
			if ( !n1 )	n1 = AddNode( vec3d(v1[0], v1[1], v1[2]), vec2d(0,0) );

			Node* n2 = FindNode( vec3d(v2[0], v2[1], v2[2]) );
			if ( !n2 )	n2 = AddNode( vec3d(v2[0], v2[1], v2[2]), vec2d(0,0) );
